...
diceContainer.addEventListener("mousedown", (event) => {
    isDragging = true;
    offsetX = event.clientX - diceContainer.getBoundingClientRect().left;
    offsetY = event.clientY - diceContainer.getBoundingClientRect().top;
...
document.addEventListener("mousemove", (event) => {
    ...
    diceContainer.style.left = `${x}px`;
    diceContainer.style.top = `${y}px`;
});
...
// Добавление события клика для броска кубика
diceContainer.addEventListener("click", () => {
    if (!isDragging) {
        // Логика броска кубика
        rollDice.click(); // Использует уже существующую функцию броска
    }
});
